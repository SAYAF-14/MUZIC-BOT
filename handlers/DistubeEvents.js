      ...
      queue.textChannel
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(`> **انتهت قائمة الانتظار لا يوجد المزيد في القائمة**`),
          ],
        })
        .then((msg) => {
          setTimeout(() => {
        ...
        .send({
          embeds: [
            new EmbedBuilder()
              .setColor(client.config.embed.color)
              .setDescription(
                `**${client.config.emoji.ERROR}لم يتم العثور على نتيجة لـ \`${quary}\`**`
              ),
          ],
        })
        .then((msg) => {
          ...
          case "autoplay":
            {
              if (!channel) {
                return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} تحتاج إلى الانضمام إلى القناة الصوتية**`
                );
              } else if (
                ...
                return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} تحتاج إلى الانضمام إلى قناتي الصوتية **`
                );
              } else if (!queue) {
                return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} لست قيد التشغيل حاليا **`
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                  `**${client.config.emoji.SUCCESS} أنت لست دي جي وأنت أيضًا لست من طلب الأغنية ..**`
                );
              } else if (!queue.autoplay) {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS} التشغيل التلقائي يعمل !! **`
                );
              } else {
                queue.toggleAutoplay();
                return send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} التشغيل التلقائي معطل !!.**`
                );
              }
            }
            break;
          case "skip":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `**${client.config.emoji.SUCCESS}  تم تخطي الأغنية !!**`
                );
              }
            }
            break;
          case "stop":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS} تم ايقاف الاغنية .**`
                );
              }
            }
            break;
          case "pauseresume":
            {
              if (!channel) {
                ...
                return send(
                  interaction,
                  `** ${client.config.emoji.ERROR} لست قيد التشغيل الان **`
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                ...
                return send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS}  تم استأناف القائمة !!** `
                );
              } else if (!queue.paused) {
                await queue.pause();
                return send(
                  interaction,
                  ` ${client.config.emoji.SUCCESS}  تم ايقاف القائمة !!** `
                );
              }
            }
            break;
          case "loop":
            {
              if (!channel) {
                return send(
                  interaction,
                  `**${client.config.emoji.ERROR} تحتاج للإنضمام لقناة صوتية**`
                );
              } else if (
                interaction.guild.members.me.voice.channel &&
                !interaction.guild.members.me.voice.channel.equals(channel)
                ...
                return send(
                  interaction,
                  `**${client.config.emoji.ERROR} أنا لست قيد التشغيل الآن**`
                );
              } else if (checkDJ) {
                return send(
                  interaction,
                ...
                return send(
                  interaction,
                  `**${client.config.emoji.SUCCESS} تكرار الاغنية يعمل !!**`
                );
              } else if (queue.repeatMode === 1) {
                await queue.setRepeatMode(2);
                return send(
                  interaction,
                  `**${client.config.emoji.SUCCESS} تكرار القائمة قيد التشغيل !!**`
                );
              } else if (queue.repeatMode === 2) {
                await queue.setRepeatMode(0);
                return send(
                  interaction,
                  `** ${client.config.emoji.SUCCESS}  التكرار معطل !!**`
                );
              }
            }
            break;
      ...
      if (message.channelId === musicchannel.id) {
        ...
        if (message.author.bot) {
          ...
          if (
            ...
            return client.embed(
              message,
              `** ${client.config.emoji.ERROR} ليس لدي إذن بالانضمام إلى قناة الصوت **`
            );
